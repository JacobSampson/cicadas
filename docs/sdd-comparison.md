...
| **Cicadas** | Explicit and programmatic: Initiative branch → Feature branches (registered) → Task branches (ephemeral). `kickoff.py` creates initiative branch; `branch.py` registers features. | `open_pr.py` supports gh / glab / Bitbucket / fallback. Blocks on BLOCK verdict from `review.md`. | `signal.py` broadcasts breaking changes to registered peer branches. `check.py` surfaces conflicts proactively. `registry.json` is the live map. |
